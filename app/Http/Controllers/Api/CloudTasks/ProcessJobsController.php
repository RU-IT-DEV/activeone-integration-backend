...
namespace App\Http\Controllers\Api\CloudTasks;
...
class ProcessJobsController extends BaseController
{
    public function queue_work(Request $request)
    {
        ...
        if (!is_null($jobClass)) {
            ...
            $job = match (true) {
                isset($payload['filePath']) => new $jobClass(
                    $payload['filePath'],
                    $payload['company'],
                    $payload['userId'],
                    $payload['userEmail'],
                ...
                isset($payload['claimsResponseId']) => new $jobClass(
                    $payload['claimsResponseId'],
                    $payload['userId']
                ),
                isset($payload['benefitPeriod']) => new $jobClass(
                    $payload['benefitPeriod']
                ),
                isset($payload['company_ids']) => new $jobClass(
                    $payload['company_ids']
                ),
